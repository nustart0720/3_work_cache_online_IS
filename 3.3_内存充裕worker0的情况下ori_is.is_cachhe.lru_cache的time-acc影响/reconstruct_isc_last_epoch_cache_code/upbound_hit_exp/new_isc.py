...
import heapq
import operator
from torchvision.datasets import folder as fld

def isc_update_cache_is(update_dict):
    # 如果在cache中，那么直接更新heap；不在cache中，和最小堆顶上的比，如果大于就替换掉，否则不动
    # 然后重新建堆

    # 先更新update_dict中存在的sample_is_key，并重新堆化；对于不存在的，然后判断是否要替换
    # for k,v in update_dict.items():
    #     if k in glv.cache:
    #         glv.cache[k] = v # 更新cache中的值
    ...
    # 先清空缓存glv.cache 然后根据update_dict中的loss降序，缓存前12500个样本
    glv.cache.clear()
    lst = sorted(update_dict.items(), key=operator.itemgetter(1), reverse=True)
    for path,imp in lst:
        glv.last_epoch_loss[path] = imp
    lst = sorted(glv.last_epoch_loss.items(), key=operator.itemgetter(1), reverse=True)
    # print(len(lst))
    cnt = 0
    for path,_ in lst:
        glv.cache[path] = fld.my_val_loader(path)
        cnt +=1
        if cnt>=12500:
            break
    # print(len(glv.cache))
