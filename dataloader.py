...
from . import IterableDataset, Sampler, SequentialSampler, RandomSampler, BatchSampler, Online_IS_Sampler
from . import _utils
...
class DataLoader(object):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None,
                 ...
                 worker_init_fn=None, multiprocessing_context=None, IS=None):
        ...
        if sampler is None:  # give default samplers
            if self._dataset_kind == _DatasetKind.Iterable:
                ...
                if shuffle:
                    if IS:
                        sampler = Online_IS_Sampler(dataset)
                    else:
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def _next_data(self):
        while True:
            ...
            if len(self._task_info[self._rcvd_idx]) == 2:
                ...
                return self._process_data(data), self.task_id_2_img_idx[self._rcvd_idx-1]  # 返回任务_rcvd_idx对应的图片的idx

            assert not self._shutdown and self._tasks_outstanding > 0
            idx, data = self._get_data()
            # print('get_data:',idx,len(data))
            # print(idx, len(data)) 任务id, [batch数据,target] 此时，对应的img_idx是self.task_id_2_img_idx[self.idx]
    ...
    def _try_put_index(self):
        ...
        try:
            ...
            # print(len(index),index) 产生一个batch的index
        ...
        self.task_id_2_img_idx[self._send_idx] = index # 任务id -> 图片idx
        # print('put_record:',self._send_idx, index[0])
